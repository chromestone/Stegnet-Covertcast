...
test_text_dataset_list = []
# if dataset is too big we might not test on all samples. prioritize most popular website first
file_list = sorted((f for f in os.listdir(os.path.join(DATA_PATH, 'html', 'test'))
					if f.startswith('file_') and f.endswith('.bin')),
					key=lambda x: '0' + x[5:6] if x[6:7] == '.' else x[5:7])
for f in file_list:
	...
	if ttd is not None:
		...
		ttd.test = True
		test_text_dataset_list.append(ttd)

test_text_dataset = ConcatDataset(test_text_dataset_list)
